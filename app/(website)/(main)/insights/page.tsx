import { getAllBlogPosts, getAllCategories, getAllTags } from "@/sanity/lib/blog";
import { BlogGrid } from "@/app/(website)/blog/_components/BlogGrid";
import { BlogFilters } from "@/app/(website)/blog/_components/BlogFilters";
import { BlogSearch } from "@/app/(website)/blog/_components/BlogSearch";
import { Pagination } from "@/app/(website)/blog/_components/Pagination";
import { Metadata } from "next";

export const revalidate = 3600;

export const metadata: Metadata = {
  title: "Insights | XMA",
  description:
    "Insights on scaling companies, growth systems, marketing infrastructure, sales pipelines, and CRM strategy from XMA.",
  openGraph: {
    title: "Insights | XMA",
    description:
      "Insights on scaling companies, growth systems, marketing infrastructure, and sales pipelines.",
    type: "website",
  },
};

interface InsightsPageProps {
  searchParams: Promise<{
    page?: string;
    search?: string;
    category?: string;
    tag?: string;
  }>;
}

export default async function InsightsPage({
  searchParams,
}: InsightsPageProps) {
  const params = await searchParams;
  const currentPage = parseInt(params.page || "1");
  const postsPerPage = 20;
  const skip = (currentPage - 1) * postsPerPage;

  const [blogData, categories, tags] = await Promise.all([
    getAllBlogPosts(postsPerPage, skip, params.search, params.category, params.tag),
    getAllCategories(),
    getAllTags(),
  ]);

  const totalPages = Math.ceil(blogData.total / postsPerPage);

  return (
    <div className="container pt-40 mx-auto px-4 py-8">
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          <h1 className="text-5xl md:text-6xl font-bold mb-6 leading-relaxed bg-gradient-to-b from-white to-gray-400 bg-clip-text text-transparent">
            Insights
          </h1>
          <p className="text-lg md:text-xl text-gray-400 max-w-3xl mx-auto leading-relaxed">
            Insights on scaling companies, growth systems, marketing
            infrastructure, and sales pipelines.
          </p>
        </div>

        <div className="mb-8">
          <BlogSearch initialValue={params.search} />
        </div>

        <div className="mb-8">
          <BlogFilters
            categories={categories}
            tags={tags}
            selectedCategory={params.category}
            selectedTag={params.tag}
          />
        </div>

        <BlogGrid posts={blogData.posts} />

        {totalPages > 1 && (
          <div className="mt-12">
            <Pagination
              currentPage={currentPage}
              totalPages={totalPages}
              baseUrl="/insights"
              searchParams={params}
            />
          </div>
        )}

        {blogData.posts.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-400 text-lg">No insights found.</p>
          </div>
        )}
      </div>
    </div>
  );
}
