...
export const WebVitalsReporter: React.FC<WebVitalsReporterProps> = ({ onMetric }) => {
  useEffect(() => {
    try {
...
export const useWebVitals = (onMetric?: (metric: Metric) => void) => {
  useEffect(() => {
    try {
...
export const performanceBudgetAlert = (metric: Metric) => {
  const rating = getVitalsRating(metric);
