...
const FeatureCard = ({
  ...
  const defaultFeatureRenderer = (feature, value) => (
    <li key={typeof feature === 'string' ? feature : feature.toString()} className="flex items-center gap-2 text-zinc-300">
      ...
      <span>{typeof feature === 'object' ? feature : `${feature}${value !== true ? `: ${value}` : ''}`}</span>
    </li>
  );
      ...
      <ul className="space-y-2">
        {Array.isArray(features)
          ...
          : Object.entries(features).map(([feature, value]) => (
              renderFeature ? renderFeature(feature, value) : defaultFeatureRenderer(feature, value)
            ))
        }
      </ul>
    </div>
